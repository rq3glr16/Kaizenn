...
// KeyBoard Options.
const int OpenMenuKey = VK_F10;
const int UninjectLibraryKey = VK_F11;
...
{
	if (spoof_call(reinterpret_cast<void*>(menu.jmp_addy), &GetAsyncKeyState, OpenMenuKey) & 0x1)
		//menu.IsOpen ? menu.IsOpen = false : menu.IsOpen = true;
		menu.IsOpen = !menu.IsOpen;
	else if(spoof_call(reinterpret_cast<void*>(menu.jmp_addy), &GetAsyncKeyState, UninjectLibraryKey) & 0x1)
	{
		...
		//DeleteFiber(g_job_pool->job_fiber);
...
DWORD __stdcall HookDX11_Init()
{
	...
	if (FAILED(hr))
	{
		g_hWnd = FindWindow(0, "PopstarV");
		sd.OutputWindow = g_hWnd;
		hr = D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, 0, levels, sizeof(levels) / sizeof(D3D_FEATURE_LEVEL), D3D11_SDK_VERSION, &sd, &g_pSwapChain, &g_pd3dDevice, &obtainedLevel, &g_pd3dContext);
		if (FAILED(hr))
		{
	...
	/*do {
		Sleep(100);
	} while (!(spoof_call(reinterpret_cast<void*>(menu.jmp_addy), &GetAsyncKeyState, UninjectLibraryKey) & 0x1));
